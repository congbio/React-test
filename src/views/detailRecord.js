...
const detailRecord = ({route} ) => {
    ...
    return (
        ...
        <TouchableOpacity style={styles.iconSetting} onPress={()=> navigation.navigate('MyRecord')}>
        ...
        <View style={styles.topDetail}>
            <ItemDetail title={'SOR Quantity Record NO'} content={route?.params?.item.number}/>
            <ItemDetail title={'SOR Quantity Record Status'} content={route?.params?.item.detail.recordStatus}/>
            <ItemDetail title={'Job'} content={route?.params?.item.detail.job}/>
            <ItemDetail title={'Foreman'} content={route?.params?.item.detail.foreman}/>
            <ItemDetail title={'Engineer'} content={route?.params?.item.detail.engneer}/>
            <ItemDetail title={'Location'} content={route?.params?.item.detail.location}/>
            <ItemDetail title={'Total Effort'} content={route?.params?.item.detail.totalEffort}/>
        </View>
        <Text style={styles.payItemTitle}>Pay Item Details</Text>
        {/* <View style={{flex:1}}> */}
        <View style={styles.bottomDetail}>
            <Text  style={styles.numberBottom} >{route?.params?.item.itemDetail.number}</Text>
            <BottomDetail 
                title1={'UoM'} content1={route?.params?.item.itemDetail.uom}
                title2={'Rate'} content2={route?.params?.item.itemDetail.rate}
            />
             <BottomDetail 
                title1={'Quantity *'} content1={route?.params?.item.itemDetail.quantity}
                title2={'Proposed Value'} content2={route?.params?.item.itemDetail.proposed}
            />
